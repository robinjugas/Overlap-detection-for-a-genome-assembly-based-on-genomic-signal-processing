...
seqs=upper(fasta);
step=10; %length of covariance computing windows
...
for i=1:N
    ...
    SequencesNumS{i,1}=zapis; %variable of numerical reads coming into overlap detection
end
    ...
    if (maxX  > threshold) || (maxY > threshold) %covariance should be higher than threshold
        ...
        if maxX > maxY %sufix joined with some prefix
            ...
            if covariance_matrix_x(rx,cx) < 0 %the LAST read is reverse complementary
                ...
                pom_contig=[ACTUAL_contig(1:length(ACTUAL_contig)-(cx*step)) GS_cumulated_phase_rcomplement(seqs{LAST})];
                ACTUAL_contig=pom_contig;
            else %is not reverse
                pom_contig=[ACTUAL_contig(1:length(ACTUAL_contig)-(cx*step)) SequencesNumS{LAST,1}];
                ACTUAL_contig=pom_contig;
            end
        ...
        elseif maxX < maxY %prefix joined with some sufix
            ...
            if covariance_matrix_y(ry,cy) < 0 %the LAST read is reverse complementary
                pom_contig=[GS_cumulated_phase_rcomplement(seqs{LAST}) ACTUAL_contig(1+cx*step:end)];
                ACTUAL_contig=pom_contig;
            else
                pom_contig=[SequencesNumS{LAST,1} ACTUAL_contig(1+cx*step:end)];
                ACTUAL_contig=pom_contig;
            end
        end
